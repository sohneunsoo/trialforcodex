...
import { getCharacterDefinitions } from '../../characters/registry'
import SystemPromptV2 from '../../constants/prompts/system-v2'

import { DEFAULT_ARTISTRY_WIDGET_SPAWNING_PROMPT } from '../../constants/prompts/character-defaults'
import { useCharacterRegistryStore } from '../character-registry'
...
export const useAiriCardStore = defineStore('airi-card', () => {
  ...
  const characterRegistryStore = useCharacterRegistryStore()
  ...
  function characterDefinitionToAiriCard(character: CharacterDefinition): AiriCard {
    return newAiriCard({
      name: character.name,
      version: '1.0.0',
      description: `Local demo character loaded from characters/${character.id}.`,
      creator: 'Local prototype',
      notes: `Prompt: ${character.promptPath}\nExpressions: ${character.expressionMapPath}\nAvatar: ${character.avatar.basePath}`,
      personality: '',
      scenario: '',
      greetings: [],
      greetingsGroupOnly: [],
  ...
  function initialize() {
    for (const character of getCharacterDefinitions())
      cards.value.set(character.id, characterDefinitionToAiriCard(character))
    ...
    const selectedCharacterId = characterRegistryStore.activeCharacterId
    if (!cards.value.has(activeCardId.value) || activeCardId.value === 'default')
      activeCardId.value = selectedCharacterId
  }
  ...
  watchDebounced(activeCard, (newCard: AiriCard | undefined) => {
    ...
    if (extension.modules?.displayModelId !== undefined) {
