...
import { useCharacterRegistryStore } from '../../stores/character-registry'
...
const emotionsQueue = createQueue<EmotionPayload>({
  handlers: [
    async (ctx) => {
      ...
      characterRegistryStore.setEmotion(ctx.data.name)
    },
  ],
})
...
watch([stageModelRenderer, () => props.paused], ([renderer]) => {
  ...
  if (renderer === 'disabled' || !renderer || !stageModelSelectedUrl.value) {
    componentState.value = 'mounted'
  }
...
watch(componentState, (nextState) => {
  characterRegistryStore.setState(nextState)
}, { immediate: true })
...
<template>
  <div relative h-full w-full>
    ...
    <div relative h-full w-full>
      ...
      />
      <div
        v-if="(stageModelRenderer === 'disabled' || !stageModelRenderer || !stageModelSelectedUrl) && showStage"
        :class="[
          'h-full w-full',
      ...
      >
        <div
          :class="[
            'relative',
            'h-[min(72vh,720px)] w-[min(80vw,520px)]',
        ...
        >
          <img
            ...
            :src="expressionImage"
            :alt="`${activeCharacter?.name ?? 'Character'} ${expression}`"
            class="character-idle-image"
            h-full max-w-full object-contain
          >
        </div>
      </div>
      <div
        v-else-if="stageModelRenderer === 'godot'"
        :class="[
          'h-full w-full',
          'flex items-center justify-center',
          'px-4 py-6',
...
.character-idle-motion {
  ...
  animation: character-idle-bob 3.8s ease-in-out infinite;
}

.character-idle-image {
  transform-origin: 50% 95%;
  animation: character-idle-sway 5.4s ease-in-out infinite;
...
@keyframes character-idle-sway {
  ...
  75% {
    transform: rotate(-1deg) scale(0.996);
  }
}
