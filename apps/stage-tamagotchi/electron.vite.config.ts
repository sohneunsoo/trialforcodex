...
function BuildInfoFallback() {
  const buildTime = new Date().toISOString()

  return {
    name: 'local-build-info-fallback',
    resolveId(id: string) {
    ...
    load(id: string) {
      if (id === '\0~build/time')
        return `export default new Date(${JSON.stringify(buildTime)})`
      if (id === '\0~build/git') {
        return [
          'export const sha = "local"',
          'export const abbreviatedSha = "local"',
          'export const branch = "local-archive"',
          `export const committerDate = ${JSON.stringify(buildTime)}`,
        ].join('\n')
      }
    },
  }
