...
import { useCharacterRegistryStore } from '@proj-airi/stage-ui/stores/character-registry'
...
} = storeToRefs(characterRegistryStore)
...
<template>
  ...
  >
    <div relative flex="~ col" z-2 h-100dvh w-100vw of-hidden>
      ...
      <HoloCoupon />
      <div
        class="absolute bottom-4 left-4 z-10 flex flex-col gap-2 rounded-lg border border-white/40 bg-white/75 px-3 py-2 text-neutral-900 shadow-lg backdrop-blur-md dark:border-neutral-800/60 dark:bg-neutral-950/70 dark:text-neutral-100"
      >
        <div text-base font-semibold leading-none>
          {{ activeCharacter?.name ?? '모노시안' }}
        </div>
        <div grid grid-cols="[auto_1fr]" gap-x-3 gap-y-1 text-xs font-mono>
          <span text-neutral-500 dark:text-neutral-400>emotion</span>
          <span>{{ emotion }}</span>
          <span text-neutral-500 dark:text-neutral-400>expression</span>
          <span>{{ expression }}</span>
          <span text-neutral-500 dark:text-neutral-400>state</span>
